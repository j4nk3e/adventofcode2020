...
local valid = 0
local total = 0
for min, max, char, pass in z:gmatch('(%d+)-(%d+) (%l): (%l+)') do
    total = total + 1
    local num = 0
    for occ in pass:gmatch(char .. '+') do num = num + #occ end
    if num >= tonumber(min) and num <= tonumber(max) then valid = valid + 1 end
end
print(valid .. '/' .. total)
valid = 0
for first, second, char, pass in z:gmatch('(%d+)-(%d+) (%l): (%l+)') do
    local num = 0
    if pass:sub(first, first) == char then num = num + 1 end
    if pass:sub(second, second) == char then num = num + 1 end
    if num == 1 then
        print(num == 1, pass:sub(first, first), pass:sub(second, second), first,
              second, char, pass)
        valid = valid + 1
    end
end
